...
export async function getCurrentPlaceforUser(position: Position): Promise<Place | null> {
    const locations: [Place] = JSON.parse(await fs.readFile(path.resolve("assets/maps/locations.json"), "utf-8"));
    for (const location of locations) {
        if (position.x > location.x && position.y > location.y
            && position.x < location.x + location.width && position.y < location.y + location.height
        ) {
            return location;
        }
    }
