...
let healitem: EquipableItem = {
    ...
    afterFight: scene1 => {
        if (Math.random() < 0.2) {
            ...
            console.log(scene1.player.stats.name + " gets healed by " + health + " from item" + healitem.name);
        }
    }
};
...
function renderStats(player: Entity) {
    ...
    return {
        ...
        value:
            `❤️HP${player.stats.health}/${player.maxhealth}
...
function renderFightEmbedded(fightscene: FightScene): JSONEncodable<APIEmbed> | APIEmbed {
    return {
        ...
        fields: [renderStats(fightscene.player), renderStats(fightscene.enemy), {
            name: "Verlauf",
            value: " "
        }]
    };
